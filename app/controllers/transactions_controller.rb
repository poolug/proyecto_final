class TransactionsController < ApplicationController
  ...
  def index
    ...
    @transactions_admin = @q.result.includes(:user).where(user_id: current_user.id).order(date_transaction: :asc).page params[:page]
  end
  ...
  def update
    respond_to do |format|
      if @transaction.update(transaction_params)
        format.html { redirect_to transactions_path, notice: "El movimiento ha sido actualizado." }
  ...
  def destroy
    @transaction.destroy
    @notice = "Movimiento eliminado."
    render "transaction_destroy_success.js.erb"
  end
