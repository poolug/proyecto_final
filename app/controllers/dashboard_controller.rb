class DashboardController < ApplicationController
  def index
    ...
    @ingresos_ultimos_meses = Transaction.group_by_month(:date_transaction, format: "%m-%Y").where('EXTRACT(MONTH FROM date_transaction) < ? ', 12.month).sum(:mount)
